...
test("a fresh directory starts empty, and flushed data survives reopening", async () => {
  ...
  const store = await GameStore.open(dir);
  ...
  await store.flush();
  const again = await GameStore.open(dir);
...
test("an unwritable directory is logged, not thrown", async () => {
  ...
  const store = await GameStore.open(path.join(file, "data"));

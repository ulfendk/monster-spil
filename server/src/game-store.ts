...
import { DEFAULT_BEAST_SETTINGS, DEFAULT_CAVE_SETTINGS, DEFAULT_DISASTER_SETTINGS, DEFAULT_ROAM_SETTINGS } from "@monster-spil/shared";
import type { AreaTerrain, BeastSettings, BeastState, CaveSettings, CaveState, DisasterKind, DisasterSettings, RaidState, RewardDelivery, RoamSettings, ScoreEvent, ScorePlayer } from "@monster-spil/shared";
...
/** Caves in the mountains: the parent's settings, when the next opens, and the open ones. */
...
const emptyCaves = (): CaveData => ({ settings: { ...DEFAULT_CAVE_SETTINGS }, active: [] });
const empty = (): GameData => ({ version: 1, players: {}, events: [], rewards: {}, renames: {}, world: emptyWorld(), roam: emptyRoam(), beasts: emptyBeasts(), caves: emptyCaves() });
...
export class GameStore {
  ...
  static async open(dir: string): Promise<GameStore> {
    try {
      ...
      return new GameStore({ ...empty(), ...raw, version: 1, world: { ...emptyWorld(), ...raw.world }, roam: { ...emptyRoam(), ...raw.roam }, beasts: { ...emptyBeasts(), ...raw.beasts }, caves: { ...emptyCaves(), ...raw.caves } }, dir);
    } catch (error) {
